...
std::any SymbolsVisitor::visitFunction(AslParser::FunctionContext *ctx) {
  ...
  // mirem si ja exiteix una declaracio amb el nom en el scope actual
  std::string funcName = ctx->ID()->getText();
  ...
  if (Symbols.findInCurrentScope(funcName)) {
    ...
    // si hi ha parametres, fem un vector amb els seus tipus
    std::vector<TypesMgr::TypeId> lParamsTy;
    if (ctx->parameters()) {
    ...
    // si hi ha un ret type el guardem, si no void
    ...
    // creem la funcio amb el seu ret type i params
    TypesMgr::TypeId tFunc = Types.createFunctionTy(lParamsTy, tRet);
    
    Symbols.addFunction(funcName, tFunc);
  }

  // creem el scope de la funcio i afegim els parametres com si fossin variables
  SymTable::ScopeId sc = Symbols.pushNewScope(funcName);
  putScopeDecor(ctx, sc);
  ...
  if (ctx->parameters()) {
    for (auto param : ctx->parameters()->parameter()) {
      ...
      TypesMgr::TypeId paramType = getTypeDecor(param->type());
      // std::cout << id << ',' << paramType << std::endl;
      // addLocalVar buscara ids duplicats als parametres
      Symbols.addLocalVar(id, paramType);
    }
  }
...
std::any SymbolsVisitor::visitVariable_decl(AslParser::Variable_declContext *ctx) {
  ...
  for (auto IDNode : ctx->ID()) {
    ...
    if (Symbols.findInCurrentScope(id)) {
      Errors.declaredIdent(IDNode);
    }
    else {
      TypesMgr::TypeId t1 = getTypeDecor(ctx->type());
      Symbols.addLocalVar(id, t1);
    }
  }
  // if (Symbols.findInCurrentScope(ident)) {
  //   Errors.declaredIdent(ctx->ID());
...
std::any SymbolsVisitor::visitFunctionCall(AslParser::FunctionCallContext *ctx) {
  DEBUG_ENTER();
